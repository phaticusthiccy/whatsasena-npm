...
async function github_user(user) {
  ...
  await axios.get('https://api.github.com/users/' + user).then(async (data_x) => {
    ...
    data.repos = data_x.data.public_repos
    data.gists = data_x.data.public_gists
    data.following = data_x.data.following
    data.follower = data_x.data.followers
    data.image = data_x.data.avatar_url
    data.id = data_x.data.id
